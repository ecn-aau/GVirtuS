...
class Endpoint_Quic : public Endpoint {
 public:
  Endpoint_Quic() = default;

  explicit Endpoint_Quic(const std::string &endp_suite,
                        const std::string &endp_protocol,
                        const std::string &endp_address,
                        const std::string &endp_port);

  Endpoint_Quic(const std::string &endp_suite)
      : Endpoint_Quic(endp_suite, "quic", "127.0.0.1", "9999") {}
   ...
   */
  Endpoint_Quic &address(const std::string &address);
   ...
   * @return reference to itself (Fluent Interface API)
   */
  Endpoint_Quic &port(const std::string &port);
 ...
 */
void from_json(const nlohmann::json &j, Endpoint_Quic &end);
}  // namespace gvirtus::communicators
